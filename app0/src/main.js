...
import { CFG, QUALITY, P, LIN, C, QS } from './core/config.js';
import { DEG, clamp } from './core/math.js';
import {
  renderer, hasFloatLinear, scene, camera, sunCam, reflCam, SUN,
  G, U, RSM, DSM, TRANSP, quadCam, quadScene, quadMesh, blit, postMat,
} from './core/engine.js';
import { SOLID_VS, SOLID_FS, PAINTED_VS, PAINTED_FS } from './core/mesh-utils.js';

import {
  HM, WS, HALF, heightData, splatData, meadowData, MEADOW_RES, RIVER_PTS,
  waterLevel, riverWidth, BRIDGE, bakeTerrain, bakeSplat, bakeMeadow, pathDistance,
  sampleHeight, buildTerrainGeometry, TERRAIN_VS, TERRAIN_FS,
  buildProxyTerrainGeometry, PROXY_TERRAIN_VS, buildRidgeBand, RIDGE_VS, RIDGE_FS,
} from './terrain.js';
import { TRACK, trackAdjust, buildTrack, trackPose, buildPermanentWay, carveTrackBed } from './railway.js';
import { BR, brPoint, pierCentres, buildBridgeCore, buildBridgeStones, roundedBoxGeometry, STONE_VS, STONE_FS } from './viaduct.js';
import { buildVillage, buildMillWheel } from './village.js';
import { finishMesh, makeTree, TREE_VS, TREE_FS, scatterTrees } from './trees.js';
import { buildRiverGeometry, WATER_VS, WATER_FS } from './river.js';
import { PUFFATLAS_FS, CLOUDSH_FS, SKY_VS, SKY_FS, buildClouds, CLOUD_VS, CLOUD_FS } from './sky.js';
import { WindSys, updateWind, windAtJS, WIND_FS, WINDVIEW_FS } from './wind.js';
import { GrassField } from './grass.js';
import { Train } from './train.js';
import {
  Particles, SMOKE_FS, MOTE_FS, BIRD_FS,
  initMotes, updateMotes, initBirds, updateBirds, updateSmoke, emitVillageSmoke,
} from './particles.js';
import { BRIGHT_FS, DOWN_FS, UP_FS, BLUR_FS, COMPOSITE_FS } from './post.js';
import { Walker } from './camera.js';
...
async function boot(){
  ...
  way.userData.depth = DSM(SOLID_VS(), U(), {side:THREE.DoubleSide});
  scene.add(way); reflectSet.push(way);
  ...
  initMotes(motes); initBirds(birdsP);
...
function frame(now){
  ...
  updateSmoke(dt, smoke); emitVillageSmoke(dt, smoke, villageSmokers);
  updateMotes(dt, camera, motes); updateBirds(dt, tAcc, birdsP);
