...
import { TAU, DEG, clamp, lerp, smoothstep, smootherstep, rng, noise2, fbm2, ridged, billow } from './core/math.js';
import { GL_UNI, GL_HASH, GL_NOISE, GL_TERRAIN, GL_WIND, GL_PAL, GL_CLOUDFIELD, GL_SHADOW, GL_LIGHT, GL_AIR } from './core/glsl.js';
// Note: bakeSplat() needs the graded track's distance field, but this module
// deliberately does NOT import from railway.js -- railway.js already imports
// BR from viaduct.js, which imports BRIDGE/riverField/etc. from *this* module
// at its own top level, so a terrain->railway edge here would close a cycle
// with a live top-level evaluation on both sides (a real TDZ hazard, not just
// a theoretical one). Instead the caller (main.js) passes TRACK in directly,
// once railway's buildTrack() has already produced it.
...
export const WATER_Y0 = 36, WATER_Y1 = 2.5;
...
export const DF = 512, DFS = WS/DF;                     // 4.69 m cells
export const dfDist = new Float32Array(DF*DF).fill(1e9);
export const dfT    = new Float32Array(DF*DF);
{
  // seed
...
export function valleyProfile(d, wobble, w){
  const bed = w*0.55;
...
export const SPAWN_KNOLL = { x:CFG.spawn.x-6, z:CFG.spawn.z-4, r:78, h:8.4 };

export function baseHills(x,z){
  const s = 0.00085;
  let h = 0;
  h += fbm2(x*s,      z*s,      4) * 46;
...
export let PATH_DF = null;
export function bakeSplat(TRACK){
  const R = CFG.dataRes;
...
export const PATH_CTRL = [
  [ 120, 30],[ 40, 62],[ -30, 96],[ -96, 120],[ -150, 136],
  [ -212, 152],[ -268, 150],[ -318, 132],[ -352, 104],
];
export const PATH_PTS = (()=>{ const a=[]; for(let i=0;i<=420;i++){ const p=catmull(PATH_CTRL,i/420); a.push(p);} return a; })();
